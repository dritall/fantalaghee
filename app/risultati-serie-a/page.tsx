  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const allEvents = [...(last?.events || []), ...(next?.events || [])];
    ...
    <div className="min-h-screen bg-black text-white p-4 pt-24 font-sans">
      <div className="max-w-5xl mx-auto">
        <div className="flex bg-zinc-900 p-1 rounded-2xl mb-8 max-w-xs mx-auto">
          {['calendario', 'classifica'].map(t => (
            <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 py-2.5 rounded-xl text-[10px] font-black uppercase ${activeTab === t ? 'bg-cyan-500 text-black' : 'text-zinc-500'}`}>{t}</button>
          ))}
        </div>
        ...
        {activeTab === 'calendario' ? (
          <div className="space-y-6">
            <div className="flex overflow-x-auto gap-2 pb-4 no-scrollbar">
              {rounds.map((_, i) => (
                <button key={i} onClick={() => setSelectedRoundIndex(i)} className={`px-4 py-2 rounded-lg shrink-0 font-bold text-xs border ${selectedRoundIndex===i?'border-cyan-400 bg-cyan-400/10':'border-white/5 text-zinc-600'}`}>G.{rounds[i][0]?.roundInfo?.round || i+1}</button>
              ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {rounds[selectedRoundIndex]?.map(m => (
                <div key={m.id} onClick={() => openMatch(m)} className="bg-zinc-900/40 border border-white/5 p-5 rounded-3xl flex justify-between items-center cursor-pointer hover:bg-zinc-800 transition-all">
                  ...
                  <div className="font-black italic">{m.status.type === 'notstarted' ? 'VS' : (m.homeScore?.current ?? 0) + "-" + (m.awayScore?.current ?? 0)}</div>
          ...
          <div className="bg-zinc-900/40 rounded-3xl p-6">
            {standings.map((t, i) => (
              <div key={t.team.id} className="flex justify-between py-4 border-b border-white/5 last:border-0 items-center px-4">
                <div className="flex items-center gap-4"><span className="text-zinc-600 font-bold">{i+1}</span><img src={getLogo(t.team.id)} className="w-6 h-6 object-contain" /><span className="text-sm font-bold">{t.team.name}</span></div>
      ...
      <Dialog.Root open={!!modalFixture} onOpenChange={() => setModalFixture(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0a0a0a] border border-white/10 rounded-[2.5rem] w-[95vw] max-w-2xl z-[101] overflow-hidden flex flex-col max-h-[85vh]">
                     ...
                     <div className="space-y-4">
                       {(matchDetails.incidents || []).map((inc: any, i: number) => (
                         <div key={i} className={`flex items-center gap-3 ${inc.isHome ? 'flex-row' : 'flex-row-reverse text-right'}`}>
                            ...
                            <div className="flex flex-col"><span className="text-xs font-bold uppercase">{inc.type === 'substitution' ? "CAMBIO" : inc.playerName}</span><span className="text-[9px] text-zinc-500">{inc.time}'</span></div>
                         </div>
                       ))}
                     </div>
                   )}
                     ...
                     <div className="grid grid-cols-2 gap-6">
                        {['home', 'away'].map(side => (
                          <div key={side} className="space-y-4">
                             <h4 className="text-[10px] font-black text-cyan-500 uppercase italic">Titolari</h4>
                             {(matchDetails.lineups[side].starters || []).map((p: any) => (
                               <div key={p.id} className="flex items-center gap-2 text-xs opacity-90"><span className="text-zinc-600 w-4">{p.number}</span><span className="uppercase">{p.name}</span></div>
                             ))}
                             <h4 className="text-[10px] font-black text-zinc-600 uppercase mt-4 italic">Panchina</h4>
                             {(matchDetails.lineups[side].subs || []).map((p: any) => (
                               <div key={p.id} className="flex items-center gap-2 text-xs opacity-60"><span className="text-zinc-700 w-4">{p.number}</span><span className="uppercase">{p.name}</span>{p.enteredAt && <ArrowUpRight className="w-2 h-2 text-emerald-400 ml-auto"/>}</div>
                             ))}
                          </div>
                        ))}
                     </div>
                          ...
                          <div key={g.groupName} className="space-y-2">
                             ...
                             {(g.statisticsItems || []).map((s: any) => {
                               ...
                               <div key={s.name} className="space-y-1">
                                  <div className="flex justify-between text-[10px] font-black italic"><span>{s.home}</span><span className="text-zinc-500 font-normal">{s.name}</span><span>{s.away}</span></div>
                                  <div className="flex h-1 bg-zinc-900 rounded-full overflow-hidden"><div className="bg-cyan-500" style={{ width: p + "%" }} /></div>
                               </div>
                             )})}
                          </div>
                        ))}
