...
export default function ScoutHub() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        console.error("🔥 [UI ERROR] Errore caricamento Hub Serie A:", e);
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-cyan-400 font-bold uppercase tracking-widest">Ottimizzazione Risorse...</div>;
    ...
    <div className="min-h-screen bg-black text-white p-4 pt-24">
        ...
        {activeTab === 'calendario' ? (
          <div className="space-y-6">
            <div className="flex overflow-x-auto gap-2 pb-4 no-scrollbar">
              {rounds.map((_, i) => (
                <button key={i} onClick={() => setSelectedRoundIndex(i)} className={`px-5 py-2 rounded-xl shrink-0 font-black text-xs border ${selectedRoundIndex===i?'border-cyan-400 bg-cyan-400/10 text-white':'border-white/5 text-zinc-600'}`}>G.{i+1}</button>
              ))}
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {rounds[selectedRoundIndex]?.map(m => (
                <div key={m.id} onClick={() => openMatch(m)} className="bg-zinc-900/40 border border-white/5 p-6 rounded-3xl flex justify-between items-center cursor-pointer hover:bg-zinc-800 transition-all">
                  <div className="flex items-center gap-3 w-[40%]"><img src={getLogo(m.homeTeam.id)} className="w-10 h-10 object-contain"/><span className="text-xs font-bold uppercase truncate">{m.homeTeam.name}</span></div>
                  <div className="text-lg font-black italic">{m.status.type === 'notstarted' ? 'VS' : (m.homeScore?.current ?? 0) + "-" + (m.awayScore?.current ?? 0)}</div>
                  <div className="flex items-center gap-3 w-[40%] justify-end text-right"><span className="text-xs font-bold uppercase truncate">{m.awayTeam.name}</span><img src={getLogo(m.awayTeam.id)} className="w-10 h-10 object-contain"/></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-zinc-900/40 rounded-3xl p-6">
            {standings.map((t, i) => (
              <div key={t.team.id} className="flex justify-between py-4 border-b border-white/5 last:border-0 items-center px-4 hover:bg-white/5 transition-colors">
                <div className="flex items-center gap-4"><span className="text-zinc-600 font-bold w-4">{i+1}</span><img src={getLogo(t.team.id)} className="w-6 h-6 object-contain" /><span className="text-sm font-bold">{t.team.name}</span></div>
      ...
      <Dialog.Root open={!!modalFixture} onOpenChange={() => setModalFixture(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#0a0a0a] border border-white/10 rounded-[3rem] w-[95vw] max-w-2xl z-[101] overflow-hidden flex flex-col max-h-[85vh]">
            <Dialog.Title className="sr-only">Match Detail</Dialog.Title>
            <Dialog.Description className="sr-only">Dettagli della partita</Dialog.Description>
            <div className="p-6 bg-white/5 border-b border-white/5">
               <div className="flex justify-between items-center mb-6 px-4">
                  ...
                  <div className="text-4xl font-black italic">{modalFixture?.homeScore?.current ?? 0} - {modalFixture?.awayScore?.current ?? 0}</div>
               ...
               <div className="flex bg-black p-1 rounded-xl w-full border border-white/5">
                     ...
                     <div className="space-y-4">
                       {matchDetails.incidents.map((inc: any, i: number) => (
                         <div key={i} className={`flex items-center gap-3 ${inc.isHome ? 'flex-row' : 'flex-row-reverse text-right'}`}>
                            ...
                            <div className="flex flex-col"><span className="text-xs font-bold uppercase">{inc.type === 'substitution' ? inc.playerInName + " ↔ " + inc.playerOutName : inc.playerName}</span><span className="text-[9px] text-zinc-500 font-bold">{inc.time}'</span></div>
                         </div>
                       ))}
                     </div>
                   )}
                          ...
                          <div key={side} className="space-y-6">
                             <div>
                                ...
                                {matchDetails.lineups[side].starters.map((p: any) => (
                                  <div key={p.id} className="flex items-center gap-2 text-xs border-b border-white/5 py-1"><span className="text-zinc-600 w-4 font-mono font-bold">{p.number}</span><span className="uppercase font-medium">{p.name}</span></div>
                                ))}
                             </div>
                             <div>
                                ...
                                {matchDetails.lineups[side].subs.map((p: any) => (
                                  <div key={p.id} className="flex items-center gap-2 text-xs opacity-60 py-1"><span className="text-zinc-700 w-4 font-mono">{p.number}</span><span className="uppercase">{p.name}</span>{p.enteredAt && <ArrowUpRight className="w-2 h-2 text-emerald-400 ml-auto"/>}</div>
                                ))}
                             </div>
                          </div>
                        ))}
                          ...
                          <div key={g.groupName} className="space-y-2">
                             ...
                             {g.statisticsItems.map((s: any) => {
                               ...
                               <div key={s.name} className="space-y-1">
                                  ...
                                  <div className="flex h-1 bg-zinc-800 rounded-full overflow-hidden"><div className="bg-cyan-500 shadow-[0_0_8px_rgba(6,182,212,0.4)]" style={{ width: p + "%" }} /></div>
                                </div>
                             )})}
                          </div>
                        ))}
            ...
            <button onClick={() => setModalFixture(null)} className="absolute top-6 right-6 p-2 bg-zinc-900 rounded-full border border-white/10"><X className="w-4 h-4"/></button>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
    </div>
