...
export function SeasonSwitcher() {
    ...
    return (
        <div ref={ref} className="relative">
            <button
                onClick={() => setOpen((v) => !v)}
                className="flex items-center gap-1 bg-white/10 border border-white/15 rounded-full pl-3 pr-2 py-1.5 text-xs font-semibold text-white whitespace-nowrap hover:bg-white/20 transition-colors"
            >
                {currentConfig.label}
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-44 rounded-xl bg-[#15102e]/95 backdrop-blur-xl border border-white/10 shadow-2xl overflow-hidden z-50">
                    {Object.values(SEASONS).map((s) => (
                        <button
                            ...
                            className={cn(
                                ...
                                current === s.slug ? "bg-white/10 text-cyan-300" : "text-white/55 hover:bg-white/5 hover:text-white"
                            )}
                        >
