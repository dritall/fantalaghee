...
export function SelettoreGiornata({
    ...
    return (
        <label
            className={cn(
                "group relative inline-flex items-center gap-2 rounded-full border border-white/12",
                "bg-white/[0.06] pl-3.5 pr-9 min-h-[42px] cursor-pointer",
                "hover:bg-white/[0.10] focus-within:border-cyan-400/45 transition-colors",
                className
            )}
        >
            <CalendarDays className="w-3.5 h-3.5 shrink-0 text-cyan-300" />
            ...
            >
                {etichettaGenerale && (
                    <option value="" className="bg-[#0b1026] text-white">
                        {etichettaGenerale}
                    </option>
                )}
                {ordinate.map((g) => (
                    <option key={g} value={g} className="bg-[#0b1026] text-white">
                        Giornata {g}
                    </option>
                ))}
            </select>

            <ChevronDown className="pointer-events-none absolute right-3.5 w-4 h-4 text-white/40 group-hover:text-white/70 transition-colors" />
        </label>
    );
}
